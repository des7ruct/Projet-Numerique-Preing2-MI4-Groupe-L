...
def Compute_tau_crossing_numeric(psi, x_array, t_array, x_barr=X_BARR, a=A_BARR):
    ...
    for n, t_val in enumerate(t_array):
        ...
        prob_trans = scipy.integrate.simpson(
            density[mask],
            x=x_array[mask]
        )
...
def plot_tunnel_snapshots(psi, potential, x_array, t_array):
    ...
    indices = [
        0,
        int(0.38 * len(t_array)),
        int(0.45 * len(t_array)),
        int(0.55 * len(t_array)),
        int(0.70 * len(t_array))
    ]
    ...
    for ax, idx in zip(axes, indices):
        ...
        trans = Compute_transmitted_probability(
            density,
            x_array,
            X_BARR + A_BARR
        )
        ...
        ax.set_title(
            f"t = {t_array[idx]:.2f}  |  P_trans = {trans:.4f}",
            fontsize=10
        )
    ...
    fig.suptitle(
        "Effet tunnel : évolution du paquet d'ondes",
        fontsize=14
    )
...
if __name__ == "__main__":
    ...
    print(f"tau0 libre analytique = {tau_free_th:.4f}")
    print(f"taut tunnel numérique = {tau_cross:.4f}")
    print(f"Probabilité transmise finale = {prob_trans:.4f}")

    print("")
    print("=== Influence de la largeur a ===")
    print("--------------------------------")
    print(" a        tau0        taut")
    print("--------------------------------")
    ...
    for a in a_values:
        ...
        psi_ini_a = Compute_ini_gaussian_wp(x)

        psi_a = Solve_schrodinger(
            psi_ini_a,
            potential_a
        )

        tau0_a = Compute_tau_free(a=a)

        taut_a = Compute_tau_crossing_numeric(
            psi_a,
            x,
            t_grid,
            a=a
